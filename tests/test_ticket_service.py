...
from database.models import Base, Department, TicketLog, User
...
class TicketServiceTest(unittest.TestCase):
    def setUp(self):
        ...
        self.db.add_all([self.department, self.creator, self.assignee])
        self.db.commit()
    ...
    def test_create_ticket_validates_foreign_keys(self):
        with self.assertRaises(ValueError):
            ts.create_ticket(self.db, title="无效创建人", description="", creator_id=999)
